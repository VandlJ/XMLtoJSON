...
namespace = {'dr': 'http://documenta.rudolphina.org/', 'xhtml': 'http://www.w3.org/1999/xhtml'}
...
def convert_to_json(xml_root, processed_links):
    ...
    ul_elements = xml_root.findall(".//xhtml:ul", namespaces=namespace)[0].findall(".//xhtml:a", namespaces=namespace)
    cnt = len(ul_elements)
    ...
    al = ArchiveLink(json_data["name"], json_data["hasSublink"], json_data["linkTo"], subs)
